...
import es.prodevelop.gvsig.mini.projection.*;
...
public class ConversionCoords {
    ...
    public static double[] geo2utm(double lon, double lat, final Elipsoide elip, final int husoforced) {
        ...
        final double CC = (0.5) * (Float11.log((1 + A) / (1 - A)));
        final double n = (Float11.atan((Math.tan(rlat)) / (Math.cos(rilon)))) - rlat;
        final double v = (c * 0.9996) / Math.sqrt((1 + se2 * Float11.pow(Math.cos(rlat), 2)));
        final double S = (se2 * Float11.pow(CC, 2) * Float11.pow(Math.cos(rlat), 2)) / 2.0;
        final double A1 = Math.sin(2 * rlat);
        final double A2 = A1 * Float11.pow(Math.cos(rlat), 2);
        final double J2 = rlat + (A1 / 2);
        final double J4 = (3 * J2 + A2) / 4;
        final double J6 = (5 * J4 + A2 * Float11.pow(Math.cos(rlat), 2)) / 3;
        final double alfa = (3.0 / 4) * se2;
        final double beta = (5.0 / 3) * Float11.pow(alfa, 2);
        final double gamma = (35.0 / 27) * Float11.pow(alfa, 3);
    ...
    public static double[] utm2geo(double x, double y, final double huso, final Elipsoide elip, final int hemisfery) {
        ...
        final double v = (c * 0.9996) / Math.sqrt((1 + se2 * Float11.pow(Math.cos(fip), 2)));
        final double aa = (x / v);
        final double A1 = Math.sin(2 * fip);
        final double A2 = A1 * Float11.pow(Math.cos(fip), 2);
        final double J2 = fip + (A1 / 2);
        final double J4 = (3 * J2 + A2) / 4;
        final double J6 = (5 * J4 + A2 * Float11.pow(Math.cos(fip), 2)) / 3.0;
        final double alfa = (3.0 / 4) * se2;
        final double beta = (5.0 / 3) * Float11.pow(alfa, 2);
        final double gamma = (35.0 / 27) * Float11.pow(alfa, 3);
        final double B = 0.9996 * c * (fip - (alfa * J2) + (beta * J4) - (gamma * J6));
        final double bb = (y - B) / v;
        final double S = (se2 * Float11.pow(aa, 2) * Float11.pow(Math.cos(fip), 2)) / 2;
        final double CC = aa * (1 - (S / 3.0));
        final double n = (bb * (1.0 - S)) + fip;
        final double sinh = (Float11.pow(Math.E, CC) - Float11.pow(Math.E, -(CC))) / 2.0;
        final double ilam = Float11.atan(sinh / Math.cos(n));
        final double tau = Float11.atan(Math.cos(ilam) * Math.tan(n));
        final double gilam = (ilam * DEGREES_PER_RADIAN);
        final double lon = gilam + lonmedia;
        final double lat = fip + (1 + se2 * (Float11.pow(Math.cos(fip), 2)) - (3.0 / 2) * se2 * Math.sin(fip) * Math.cos(fip) * (tau - fip)) * (tau - fip);
    ...
    public static double distanciaProj(final double lon1, final double lat1, final double lon2, final double lat2, final Elipsoide elip) {
        ...
        final double dist = Math.sqrt((Float11.pow((p2[0] - p1[0]), 2)) + (Float11.pow((p2[1] - p1[1]), 2)));
        return dist;
    }
...
//        double fscale = ((-9.39) / 1000000.0);
    ...
    public static double[] PIGbsl(final double lon1_, final double lat1_,
        ...
        final double e4 = Float11.pow(e2, b);
        final double se = elip.getSe();

        /* sphere latitudes */
        final double u1 = Float11.atan(b / a * Math.tan(_lat1));
        final double u2 = Float11.atan(b / a * Math.tan(_lat2));
        ...
        for (int i = 0; i < ITERATIONS; i++) {
            ...
            a12 = Float11.atan(g1) + Float11.atan(g2);
            a21 = Float11.atan(g2) - Float11.atan(g1) + Math.PI;
            ...
            M = Float11.atan((Math.tan(u1)) / (Math.cos(a12)));
            m = Float11.acos((Math.sin(u1)) / (Math.sin(M)));
            sig = Float11.acos(Math.sin(u1) * Math.sin(u2) + Math.cos(u1) * Math.cos(u2) * Math.cos(wt));
            t11 = e2 / 8;
            t12 = e2 * Float11.pow(Math.cos(m), 2) / 16;
            t1 = (e2 * Math.sin(m) * sig * (0.5 + t11 - t12));
            t2 = e4 * Math.sin(m) * (Float11.pow(Math.cos(m), 2) * Math.sin(sig) * Math.cos(2 * M + sig)) / 16;
            ...
            if (condi <= threshold) {
                ...
                final double tA = 1 + (Float11.pow(k, 2) / 4 - 3 * Float11.pow(k, 4) / 64);
                final double tB = Float11.pow(k, 2) / 4 - Float11.pow(k, 4) / 16;
                final double tC = Float11.pow(k, 4) / 128;
    ...
    public static double[] geo2mercator(final double lon, final double lat, final Elipsoide elip) {
        ...
        double y = Float11.pow(((1 - (Math.sqrt(ep2) * sl)) / (1 +
                (Math.sqrt(ep2) * sl))), (Math.sqrt(ep2)));
        y = a / 2 * (Float11.log(tl * y));
    ...
    public static double[] newgeo2mercator(final double lon, final double lat) {
        ...
        final double y = 0.5 * Float11.log((1 + Math.sin(rlat)) / (1 - Math.sin(rlat)));
        // double y = 0.5 * 
        // Float11.atan(sinh(rlat));
        // double y = Float11.log(Math.tan(phi) + sec(phi));
    ...
    public static double[] newmercator2geo(final double lon, final double lat) {
        ...
        final double y = 2.0 * Float11.atan(Float11.exp(rlat)) - 0.5 * Math.PI;
        // double y = 0.5 * 
        // Float11.atan(sinh(rlat));
        // double y = Float11.log(Math.tan(phi) + sec(phi));
...
//                    resp[1] = lat;
    ...
    private static double[] wgs84toLambert2e(final double lon, final double lat) {
        ...
        final double N = a_w / Math.sqrt(1 - e2_w * Float11.pow(Math.sin(phi_w), 2));
        ...
        double p0 = Float11.atan(Z_n / Math.sqrt(X_n * X_n + Y_n * Y_n) * (1 - (a_n * e2_n) / (Math.sqrt(X_n * X_n + Y_n * Y_n + Z_n * Z_n))));
        double p1 = Float11.atan((Z_n / Math.sqrt(X_n * X_n + Y_n * Y_n)) / (1 - (a_n * e2_n * Math.cos(p0)) / (Math.sqrt((X_n * X_n + Y_n * Y_n) * (1 - e2_n * Float11.pow(Math.sin(p0), 2))))));

        while (!(Math.abs(p1 - p0) < epsilon)) {
            p0 = p1;
            p1 = Float11.atan((Z_n / Math.sqrt(X_n * X_n + Y_n * Y_n)) / (1 - (a_n * e2_n * Math.cos(p0)) / (Math.sqrt((X_n * X_n + Y_n * Y_n) * (1 - e2_n * Float11.pow(Math.sin(p0), 2))))));
        }

        final double phi_n = p1;
        final double lambda_n = Float11.atan(Y_n / X_n);
        ...
        final double L = Float11.log(Math.tan(Math.PI / 4 + phi_n / 2) * Float11.pow(((1 - e_n * Math.sin(phi_n)) / (1 + e_n * Math.sin(phi_n))), (e_n / 2)));

        final double X_l2e = Xs + c * Float11.exp((-n * L)) * Math.sin(n * (lambda_n - lambda0));
        final double Y_l2e = Ys - c * Float11.exp((-n * L)) * Math.cos(n * (lambda_n - lambda0));
    ...
    private static double northFromLonLat(
        ...
        final double e2 = (Float11.pow(af0, 2) - Float11.pow(bf0, 2)) / Float11.pow(af0, 2);
        final double n = (af0 - bf0) / (af0 + bf0);
        final double nu = af0 / (Math.sqrt(1 - (e2 * Float11.pow(Math.sin(RadPHI), 2))));
        final double rho = (nu * (1 - e2)) / (1 - (e2 * Float11.pow(Math.sin(RadPHI), 2)));
        ...
        final double III = ((nu / 24) * (Math.sin(RadPHI)) * (Float11.pow(Math.cos(RadPHI), 3))) * (5 - (Float11.pow(Math.tan(RadPHI), 2)) + (9 * eta2));
        final double IIIA = ((nu / 720) * (Math.sin(RadPHI)) * (Float11.pow(Math.cos(RadPHI), 5))) * (61 - (58 * (Float11.pow(Math.tan(RadPHI), 2))) + (Float11.pow(Math.tan(RadPHI), 4)));

        return I + (Float11.pow(p, 2) * II) + (Float11.pow(p, 4) * III) + (Float11.pow(p, 6) * IIIA);
    }
    ...
    private static double eastFromLonLat(
        ...
        final double V = (nu / 6) * (Float11.pow(Math.cos(RadPHI), 3)) * ((nu / rho) -
                (Float11.pow(Math.tan(RadPHI), 2)));
        final double VI = (nu / 120) * (Float11.pow(Math.cos(RadPHI), 5)) * (5 -
                (18 * (Float11.pow(Math.tan(RadPHI), 2))) + (Float11.pow(Math.tan(RadPHI), 4)) + (14 * eta2) - (58 * (Float11.pow(Math.tan(RadPHI), 2)) * eta2));

        return e0 + (p * IV) + (Float11.pow(p, 3) * V) + (Float11.pow(p, 5) * VI);
    }
    ...
    private static double marc(final double bf0, final double n, final double PHI0, final double PHI) {
        ...
        return bf0 * (((1 + n + ((5 / 4) * Float11.pow(n, 2)) + ((5 / 4) * Float11.pow(n, 3))) * (PHI - PHI0)) - (((3 * n) + (3 * Float11.pow(n, 2)) + ((21 / 8) * Float11.pow(n, 3))) * (Math.sin(PHI - PHI0)) * (Math.cos(PHI + PHI0))) + ((((15 / 8) * Float11.pow(n, 2)) + ((15 / 8) * Float11.pow(n, 3))) * (Math.sin(2 * (PHI - PHI0))) * (Math.cos(2 * (PHI + PHI0)))) - (((35 / 24) * Float11.pow(n, 3)) *
    ...
    private static double[] WGS84toLambertZoneI(final double lon, final double lat) {
        ...
        final double X_lz1 = Xs + c * Float11.exp((-n * L)) * Math.sin(n * (lambda_n - lambda0));
        final double Y_lz1 = Ys - c * Float11.exp((-n * L)) * Math.cos(n * (lambda_n - lambda0));
    ...
    private static double[] WGS84toLambertZoneII(final double lon, final double lat) {
        ...
        final double epsilon = Float11.pow(10, -10);
        ...
        final double X_lz2 = Xs + c * Float11.exp((-n * L)) * Math.sin(n * (lambda_n - lambda0));
        final double Y_lz2 = Ys - c * Float11.exp((-n * L)) * Math.cos(n * (lambda_n - lambda0));
    ...
    private static double[] WGS84toLambertZoneIII(final double lon, final double lat) {
        ...
        final double X_lz3 = Xs + c * Float11.exp((-n * L)) * Math.sin(n * (lambda_n - lambda0));
        final double Y_lz3 = Ys - c * Float11.exp((-n * L)) * Math.cos(n * (lambda_n - lambda0));
    ...
    private static double[] WGS84toLambertZoneIV(double lon, double lat) {
        ...
        final double X_lz4 = Xs + c * Float11.exp((-n * L)) * Math.sin(n * (lambda_n - lambda0));
        final double Y_lz4 = Ys - c * Float11.exp((-n * L)) * Math.cos(n * (lambda_n - lambda0));
    ...
    /**
     * Reprojects the coordinates from on projection to another
     * @param lon Longitude
     * @param lat Latitude
     * @param from The projection in which are the origin coordinates
     * @param to The projection to reproject
     * @return The coordinates reprojected
     */
